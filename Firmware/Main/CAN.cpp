...
/* Handle for a queue storing received CAN messages */
QueueHandle_t messageQueue;

/* Stores the latest message pulled from the queue */
...
void addToQueue(const CAN_message_t &msg) {
  queue_message q_msg;
  q_msg.id = msg.id;
  for (int i = 0; i < msg.len; i++) {
    q_msg.buf[i] = msg.buf[i];
  }
  xQueueSendToBackFromISR(messageQueue, &q_msg, NULL);
}

void decipherEVCCStats(queue_message msg, ChargeControllerStats *evcc_stats) {
...
void decipherChargerStats(queue_message msg, ChargerStats *charger_stats) {
...
void decodeMotorStats(queue_message msg, MotorStats *motor_stats) {
...
void decodeMotorTemps(queue_message msg, MotorTemps *motor_temps) {
...
void decipherBMSStatus(queue_message msg, BMSStatus *bms_status) {
...
void decipherCellsVoltage(queue_message msg, BatteryVoltages *battery_voltages) {
...
void decipherThermistors(queue_message msg, ThermistorTemps *thermistor_temps) {
...
static void checkCAN(CANTaskData canData) {
  ...
  switch (received_msg.id) {
    case MOTOR_STATS_MSG:
      decodeMotorStats(received_msg, &(context->motor_stats));
      break;
    case MOTOR_TEMPS_MSG:
      decodeMotorTemps(received_msg, &(context->motor_temps));
      break;
    case DD_BMS_STATUS_IND:
      decipherBMSStatus(received_msg, &(context->bms_status));
      //printBMSStatus(context->bms_status);
      break;
    case EVCC_STATS:
      decipherEVCCStats(received_msg, &(context->charge_controller_stats));
      break;
    case CHARGER_STATS:
      decipherChargerStats(received_msg, &(context->charger_stats));
    case BMSC1_LTC1_CELLS_04:
      decipherCellsVoltage(received_msg,  &(context->battery_voltages));
      break;
    ...
    case DD_BMSC_TH_STATUS_IND:
      decipherThermistors(received_msg, &(context->thermistor_temps));
      break;
  }
}
...
void canReceiveTask(void *canData) {
  messageQueue = xQueueCreate(16, sizeof(queue_message));
  ...
  CAN_bus.onReceive(addToQueue);
  while (1) {
    if (xQueueReceive(messageQueue, &received_msg, portMAX_DELAY) == pdPASS) {
